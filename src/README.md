...
`src/app/__main__.py` binds to a random localhost port, prints `PORT=<n>` to stdout (read by Tauri), then starts uvicorn. Heavy imports (torch, msclap) happen *after* the port is printed so Tauri can connect immediately.
...
From [`CLAUDE.md`](../CLAUDE.md#critical-rules):
...
- All AI suggestions are proposals — nothing written until the user explicitly saves.
...
`app/paths.py` resolves the correct path at runtime. See [`docs/04-data-reference-doc.md`](../docs/04-data-reference-doc.md) for the required files.
