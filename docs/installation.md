...
| **RAM**      | 8 GB minimum, 16 GB recommended (ML analysis loads ~1.5 GB model weights)                                                                  |
| **Disk**     | 4 GB free for application + ML model weights                                                                                               |
...
2. **Begin loading ML models in the background.** The status bar at the bottom shows the loading state. The app is fully usable as a manual metadata editor while models load.
...
| ClapCap + GPT-2 (description generator) | ~2.1 GB | Downloaded the first time you click **Generate** with ML descriptions enabled |
...
| `settings.json`             | App settings (creator ID, library name, ML preferences)        |
| `cache\text_embeddings.npz` | Pre-computed ML label embeddings (generated on first analysis) |
...
2. **Analyze** — Select one or more files and click the **Generate** button (⚡). The ML analyzes the audio and suggests a UCS category, subcategory, description, keywords, and filename.
...
2. **ClapCap model download** — ~2.1 GB downloaded from Hugging Face the first time ML descriptions are generated. Subsequent launches skip both steps.
